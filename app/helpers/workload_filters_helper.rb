...
module WorkloadFiltersHelper
	...
	def wl_zoom_link(zoom, in_or_out)
		...
		when :in
		  if zoom < 4
			link_to_content_update l(:text_zoom_in), params.merge({:zoom => (zoom + 1)}), :class => 'icon icon-zoom-in'
		  else
		...
		when :out
		  if zoom > 2
			link_to_content_update l(:text_zoom_out), params.merge({:zoom => (zoom - 1)}), :class => 'icon icon-zoom-out'
		  else
